...
void main() {
  group('EmbeddedSignalingServer Tests', () {
    ...
    test('accepts connection with valid PIN in query parameter', () async {
      ...
      final clientWs = await WebSocket.connect('ws://127.0.0.1:$port/ws?pin=4829');
      await Future.delayed(const Duration(milliseconds: 100));
    ...
    test('rejects connection with invalid PIN in query parameter with HTTP 401', () async {
      final port = await server.start(port: 18094, requiredPin: '4829');

      expect(
        () async => await WebSocket.connect('ws://127.0.0.1:$port/ws?pin=9999'),
        throwsA(anything),
      );
      await Future.delayed(const Duration(milliseconds: 100));
      expect(server.hasConnectedClient, isFalse);
    ...
    test('verifies in-band auth handshake when connecting without URL PIN', () async {
      ...
      final responseStr = await clientWs.first.timeout(const Duration(seconds: 2));
