...
class ReceiverWebRTCService {
  ...
  final StreamController<StreamSourceType> _streamSourceController =
      StreamController<StreamSourceType>.broadcast();
  ...
  Stream<StreamSourceType> get streamSourceStream =>
      _streamSourceController.stream;
  ...
  Future<void> _setupPeerConnection() async {
    ...
    _peerConnection!.onIceCandidate = (RTCIceCandidate candidate) async {
      if (candidate.candidate != null && candidate.candidate!.isNotEmpty) {
        debugPrint(
          '[ReceiverWebRTC] Raw onIceCandidate received: ${candidate.candidate}',
        );
        final myIp =
        ...
        if (fixedCandStr != null) {
          debugPrint(
            '[ReceiverWebRTC] Local ICE Candidate generated: $fixedCandStr',
          );
    ...
    _peerConnection!.onTrack = (RTCTrackEvent event) async {
      ...
      if (event.track.kind == 'video') {
        if (event.streams.isNotEmpty) {
          ...
          try {
            if (_fallbackRemoteStream != null) {
              await _fallbackRemoteStream!.dispose();
            }
            _fallbackRemoteStream = await createLocalMediaStream(
              'remote_stream',
            );
            await _fallbackRemoteStream!.addTrack(event.track);
            renderer.srcObject = _fallbackRemoteStream;
          } catch (e) {
            debugPrint(
              '[ReceiverWebRTC] Error creating local stream for track: $e',
            );
          }
        }
        _streamingStatusController.add(true);
      ...
      switch (message.type) {
        case 'offer':
          if (message.sdp != null) {
            if (_isProcessingOffer) {
              debugPrint(
                '[ReceiverWebRTC] Already processing an offer, ignoring duplicate.',
              );
              return;
            }
            ...
            try {
              ...
              final detectedEngine = SdpCandidateSanitizer.detectCodecEngine(
                message.sdp,
              );
              debugPrint(
              ...
              debugPrint(
                '[ReceiverWebRTC] <<< INCOMING SANITIZED OFFER:\n$sanitizedOfferSdp',
              );
              ...
              for (final candidate in queued) {
                ...
                try {
                  ...
                  debugPrint(
                    '[ReceiverWebRTC] Error adding queued candidate: $e',
                  );
                }
              }
              ...
              signalingServer.sendMessage(SignalingMessage.answer(sdpStr));
            } finally {
            ...
            if (fixedCandStr != null) {
              ...
              if (_hasRemoteDescription && _peerConnection != null) {
                ...
                try {
                  ...
                  debugPrint(
                    '[ReceiverWebRTC] Error adding direct candidate: $e',
                  );
                }
              } else {
                debugPrint('[ReceiverWebRTC] Queuing candidate: $fixedCandStr');
                if (_iceCandidateQueue.length < 50) {
        ...
        case 'thermal_telemetry':
          if (message.payload != null) {
            _senderTemperatureC = (message.payload!['temperatureC'] as num?)
                ?.toDouble();
        ...
        case 'stream_metadata':
          if (message.payload != null) {
            final sourceName = message.payload!['streamSource'] as String?;
            if (sourceName != null) {
              final source = StreamSourceType.values.firstWhere(
                (e) => e.name == sourceName,
                orElse: () => StreamSourceType.screen,
              );
              _streamSourceController.add(source);
            }
          }
          break;
  ...
  void _startStatsPolling() {
    ...
    _statsTimer = Timer.periodic(const Duration(milliseconds: 1500), (_) async {
      ...
      try {
        ...
        final metrics = _statsCalculator.calculateInbound(
          reports,
          fallbackWidth: renderer.videoWidth > 0 ? renderer.videoWidth : 1280,
          fallbackHeight: renderer.videoHeight > 0 ? renderer.videoHeight : 720,
        );
  ...
  Future<void> _resetPeerConnection() async {
    ...
    try {
      if (_fallbackRemoteStream != null) {
        try {
          for (final track in _fallbackRemoteStream!.getTracks()) {
            await track.stop();
          }
          await _fallbackRemoteStream!.dispose();
        } catch (_) {}
  ...
  Future<void> dispose() async {
    ...
    await _streamSourceController.close();
